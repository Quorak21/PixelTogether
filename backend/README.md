...
1. **Serveur principal** : [Express](./index.js) n'est utilisé que comme coquille vide pour le routage HTTP de base et la gestion de la sécurité CORS.
2. **Communication en temps réel** : Tout passe par [Socket.io](./app/createServer.js). C'est lui qui gère le chat, la pose des pixels et la synchronisation des écrans.
3. **Base de données** : Il n'y a **pas de base de données persistante** (pas de SQL ni de NoSQL) ! Tout est stocké en mémoire vive dans l'objet global `activeEvents` du fichier [eventStore.js](./store/eventStore.js). Si le serveur redémarre, toutes les parties en cours sont perdues.
...
Chaque salon de jeu créé est modélisé par un objet `Event` stocké dans la base en mémoire d' [eventStore.js](./store/eventStore.js). Voici à quoi il ressemble :
...
### 1. Base de données en mémoire ([eventStore.js](./store/eventStore.js))
...
### 2. Attribution des couleurs ([colorSplit.js](./services/colors/colorSplit.js))
* **`splitPalette(pool, playerCount)`** : Reçoit la palette globale de 16 couleurs et le nombre de joueurs. Divise le tableau en parts égales. S'il y a un reste de division (ex: 16 couleurs pour 3 joueurs, reste 1), elle distribue le reste équitablement aux premiers joueurs.
  * *Lien avec* : `shuffleArray` de [groupShuffle.js](./services/shuffle/groupShuffle.js) pour mélanger les couleurs avant découpe.
* **`assignPalettesToGroup(group)`** : Mélange la liste des joueurs d'un groupe et attribue à chacun son lot de couleurs exclusives. Ainsi, deux équipiers ne partagent aucune couleur sur la palette de dessin.
  * *Lien avec* : `GAME_PALETTE_16` dans [constants.js](./config/constants.js).

---

### 3. Logique des groupes ([groupShuffle.js](./services/shuffle/groupShuffle.js))
...
### 4. Configuration des modes ([gameMode.js](./services/event/gameMode.js))
...
### 5. Gestion des sessions de reconnexion ([sessionToken.js](./services/reconnect/sessionToken.js))
...
### 6. Gestion des participants ([participants.js](./services/event/participants.js))
...
* **`remapSocket(event, playerId, newSocketId)`** : Réassocie les nouveaux IDs de socket dans toutes les listes internes (joueurs de la room, joueurs du groupe, session active) après une reconnexion.
  * *Lien avec* : `setSessionConnected` dans [sessionToken.js](./services/reconnect/sessionToken.js).
...
* **`isManagerConnected(event)`** : Indique si la session manager est connectée (via `sessionToken`).
  ...
  * *Lien avec* : `closeEvent` dans [lifecycle.js](./services/event/lifecycle.js).

---

### 7. Déroulement du dessin ([sessionLifecycle.js](./services/session/sessionLifecycle.js))
...
### 7b. Fin de groupe compétitif ([groupFinish.js](./services/session/groupFinish.js))
...
* **`canAccessLobby(event, playerId)`** / **`isGroupSpectator(event, socket, group)`** : Autorisent l'accès au lobby joueur et le mode spectateur (chat sans historique, pas de pixel). Voir aussi [groupAccess.js](./services/session/groupAccess.js).

---

### 8. Chronomètres de session ([sessionTimer.js](./services/session/sessionTimer.js))
...
### 9. Rendu d'images ([gridPreview.js](./services/grid/gridPreview.js) & [preview.js](./services/grid/preview.js))
...
### 10. Formatage des données de vote ([voteLifecycle.js](./services/vote/voteLifecycle.js))
...
1. **Pas de persistence** : N'essaie pas de chercher de base de données SQL ou Mongo, tout est dans `activeEvents` dans [eventStore.js](./store/eventStore.js). Si tu as besoin d'ajouter un attribut persistant à un salon ou un joueur, rajoute-le à la création de l'objet dans `newGrid` ([lobby.handlers.js](./sockets/handlers/lobby.handlers.js)) ou `registerPlayer` ([waitingRoom.handlers.js](./sockets/handlers/waitingRoom.handlers.js)).
...
3. **Throttling (Limitation de fréquence)** : Pour éviter que le serveur ne sature si un joueur clique 100 fois par seconde ou envoie un script pour spammer le canvas, utilise `isRateLimited(socket, 'nomAction', délaiMs)` de [socketGuards.js](./sockets/handlers/socketGuards.js).
