...
// vue joueur : attente des autres groupes + spectate les grilles encore actives
...
export class LobbyPageComponent {
  ...
  readonly isPlayerLobby = computed(() => !this.ui.isManager());

  readonly lobbyTitle = computed(() =>

    this.isPlayerLobby() ? 'En attente des autres groupes' : this.managerPartyName() || 'Supervision',
  ...
  private async tryReconnectLobby(): Promise<boolean> {
    ...
    if (response.phase === 'lobby' && response.lobbyState) {
      ...
      this.applyLobbyState(response.lobbyState, response.role);
  ...
  private applyLobbyState(response: EventLobbyStatePayload, role?: 'manager' | 'player'): void {
  ...
  private bindManagerListeners(): void {
    ...
    const onLobbyGroupsUpdated = (payload: LobbyGroupsUpdatedPayload) => {
      ...
      this.managerGroups.set(payload.groups ?? []);
    ...
    this.socket.on<LobbyGroupsUpdatedPayload>('lobbyGroupsUpdated', onLobbyGroupsUpdated);
    ...
    this.destroyRef.onDestroy(() => {
      ...
      this.socket.off('lobbyGroupsUpdated', onLobbyGroupsUpdated as (...args: unknown[]) => void);
